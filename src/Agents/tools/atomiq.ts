...
export class AtomiqTool extends BaseTool<AtomiqPayload> {
  metadata: ToolMetadata = {
    name: 'atomiq_tool',
    description:
      'Cross-chain swap operations via Atomiq - Bitcoin to Starknet swaps, Lightning Network swaps, and cross-chain transactions',
    parameters: {
      operation: {
        type: 'string',
        description:
          "The operation to perform (e.g., 'create_swap', 'get_swap_by_id', 'get_refundable_swaps', 'get_spendable_balance', 'parse_address', 'get_bitcoin_spendable_balance', 'get_swap_limits')",
        required: true,
        enum: [
          'create_swap',
          'get_swap_by_id',
          'get_refundable_swaps',
          'get_spendable_balance',
          'parse_address',
          'get_bitcoin_spendable_balance',
          'get_swap_limits',
        ],
      },
      fromAsset: {
      ...
      fromChain: {
        type: 'string',
        description:
          "The blockchain to swap from (e.g., 'bitcoin', 'starknet')",
        required: false,
      },
      toChain: {
        type: 'string',
      ...
      signer: {
        type: 'object',
        description:
          'The signer object for spendable balance (e.g., a wallet instance)',
        required: false,
      },
      token: {
        type: 'object',
        description:
          'The token object for spendable balance (e.g., Atomiq.Tokens.BITCOIN.BTC)',
        required: false,
      },
    },
    examples: [
  ...
  async execute(payload: AtomiqPayload, userId: string): Promise<ToolResult> {
    ...
    try {
      ...
      memoryStore.add(
        userId,
        `Error in AtomiqTool: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
  ...
  private async createSwap(payload: AtomiqPayload): Promise<ToolResult> {
    ...
    if (typeof userAddress === 'object' && userAddress !== null) {
      userAddress =
        (userAddress as any).address || (userAddress as any).userAddress;
    }
    ...
    if (
      ...
      return this.createErrorResult(
        'atomiq_swap',
        'From asset, to asset, amount, user address, and recipient address are required for swap'
      );
    }
    ...
    try {
      ...
      if (payload.fromAsset === 'BTC' && payload.toAsset === 'STRK') {
        ...
        return this.createErrorResult(
          'atomiq_swap',
          `Unsupported asset pair: ${payload.fromAsset} -> ${payload.toAsset}`
        );
      }
  ...
  private async getSwapById(payload: AtomiqPayload): Promise<ToolResult> {
    ...
    try {
      ...
      return this.createErrorResult(
        'atomiq_swap',
        `Failed to get swap: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  private async getRefundableSwaps(
    payload: AtomiqPayload
  ): Promise<ToolResult> {
    if (!payload.chain || !payload.address) {
      return this.createErrorResult(
        'atomiq_refundable_swaps',
        'Chain and address are required'
      );
    }

    try {
      const refundableSwaps = await this.atomiqService.getRefundableSwaps(
        payload.chain,
        payload.address
      );
      ...
      return this.createErrorResult(
        'atomiq_refundable_swaps',
        `Failed to get refundable swaps: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  private async getSpendableBalance(
    payload: AtomiqPayload
  ): Promise<ToolResult> {
    if (!payload.signer || !payload.token) {
      return this.createErrorResult(
        'atomiq_spendable_balance',
        'Signer and token are required'
      );
    }

    try {
      const spendableBalance = await this.atomiqService.getSpendableBalance(
        payload.signer,
        payload.token
      );
      ...
      return this.createErrorResult(
        'atomiq_spendable_balance',
        `Failed to get spendable balance: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async parseAddress(payload: AtomiqPayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult(
        'atomiq_parse_address',
        'Address is required'
      );
    }

    try {
      const parsedAddress = await this.atomiqService.parseAddress(
        payload.address
      );
      ...
      return this.createErrorResult(
        'atomiq_parse_address',
        `Failed to parse address: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }

  private async getBitcoinSpendableBalance(
    payload: AtomiqPayload
  ): Promise<ToolResult> {
    if (!payload.bitcoinAddress || !payload.destinationChain) {
      return this.createErrorResult(
        'atomiq_bitcoin_spendable_balance',
        'Bitcoin address and destination chain are required'
      );
    }

    try {
      const bitcoinSpendable =
        await this.atomiqService.getBitcoinSpendableBalance(
      ...
      return this.createErrorResult(
        'atomiq_bitcoin_spendable_balance',
        `Failed to get Bitcoin spendable balance: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async getSwapLimits(payload: AtomiqPayload): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult(
        'atomiq_limits',
        `Failed to get swap limits: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
}
