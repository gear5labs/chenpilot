...
export class TrovesTool extends BaseTool<TrovesPayload> {
  metadata: ToolMetadata = {
    name: "troves_tool",
    description: "Yield farming operations via Troves - deposit, withdraw, harvest yields, and manage positions",
    parameters: {
      operation: {
        type: "string",
  ...
  private async getStrategies(): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("troves_strategies", `Failed to get strategies: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getStrategyDetails(payload: TrovesPayload): Promise<ToolResult> {
    if (!payload.strategyId) {
      return this.createErrorResult("troves_strategy", "Strategy ID is required");
    }
    ...
    try {
      ...
      if (!strategy) {
        return this.createErrorResult("troves_strategy", `Strategy ${payload.strategyId} not found`);
      }
      ...
      return this.createErrorResult("troves_strategy", `Failed to get strategy details: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getVaults(): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("troves_vaults", `Failed to get vaults: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getVaultDetails(payload: TrovesPayload): Promise<ToolResult> {
    ...
    try {
      ...
      if (!vault) {
        return this.createErrorResult("troves_vault", `Vault ${payload.vaultId} not found`);
      }
  ...
  private async deposit(payload: TrovesPayload): Promise<ToolResult> {
    if (!payload.vaultId || !payload.amount || !payload.asset) {
      return this.createErrorResult("troves_deposit", "Vault ID, amount, and asset are required for deposit");
    }
    ...
    try {
      ...
      return this.createErrorResult("troves_deposit", `Failed to prepare deposit: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async withdraw(payload: TrovesPayload): Promise<ToolResult> {
    if (!payload.vaultId || !payload.amount) {
      return this.createErrorResult("troves_withdraw", "Vault ID and amount are required for withdrawal");
    }
    ...
    try {
      ...
      return this.createErrorResult("troves_withdraw", `Failed to prepare withdrawal: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async harvest(payload: TrovesPayload): Promise<ToolResult> {
    if (!payload.vaultId) {
      return this.createErrorResult("troves_harvest", "Vault ID is required for harvest");
    }
    ...
    try {
      ...
      return this.createErrorResult("troves_harvest", `Failed to prepare harvest: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getPositions(payload: TrovesPayload): Promise<ToolResult> {
    if (!payload.userAddress) {
      return this.createErrorResult("troves_positions", "User address is required for positions");
    }

    try {
      const positions = await this.trovesService.getUserPositions(payload.userAddress);
      ...
      return this.createErrorResult("troves_positions", `Failed to get positions: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getPositionDetails(payload: TrovesPayload): Promise<ToolResult> {
    if (!payload.positionId) {
      return this.createErrorResult("troves_position", "Position ID is required");
    }

    try {
      // Get all positions and find the one with matching ID
      const positions = await this.trovesService.getUserPositions(payload.userAddress || "");
      const position = positions.find(p => (p as any).id === payload.positionId);
      if (!position) {
        return this.createErrorResult("troves_position", `Position ${payload.positionId} not found`);
      }
      ...
      return this.createErrorResult("troves_position", `Failed to get position details: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getYieldData(payload: TrovesPayload): Promise<ToolResult> {
    if (!payload.vaultId) {
      return this.createErrorResult("troves_yield", "Vault ID is required for yield data");
    }
    ...
    try {
      ...
      return this.createErrorResult("troves_yield", `Failed to get yield data: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getQuotes(payload: TrovesPayload): Promise<ToolResult> {
    if (!payload.vaultId || !payload.amount || !payload.asset) {
      return this.createErrorResult("troves_quotes", "Vault ID, amount, and asset are required for quotes");
    }

    try {
      const quotes = await this.trovesService.getDepositQuote(payload.vaultId, payload.amount.toString(), payload.asset);
      ...
      return this.createErrorResult("troves_quotes", `Failed to get quotes: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getHealthCheck(payload: TrovesPayload): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("troves_health", `Failed to get health check: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getSupportedAssets(): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("troves_assets", `Failed to get supported assets: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getVaultAPY(payload: TrovesPayload): Promise<ToolResult> {
    if (!payload.vaultId) {
      return this.createErrorResult("troves_apy", "Vault ID is required for APY");
    }
    ...
    try {
      ...
      const vault = vaults.find(v => v.id === payload.vaultId);
      if (!vault) {
        return this.createErrorResult("troves_apy", `Vault ${payload.vaultId} not found`);
      }
      ...
      return this.createErrorResult("troves_apy", `Failed to get vault APY: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getUserBalance(payload: TrovesPayload): Promise<ToolResult> {
    if (!payload.userAddress) {
      return this.createErrorResult("troves_balance", "User address is required for balance");
    }

    try {
      // Get user positions to calculate balance
      const positions = await this.trovesService.getUserPositions(payload.userAddress);
      const totalBalance = positions.reduce((sum, pos) => sum + ((pos as any).balance || 0), 0);
      ...
      return this.createErrorResult("troves_balance", `Failed to get user balance: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getTotalValueLocked(): Promise<ToolResult> {
    try {
      ...
      const vaults = await this.trovesService.getAvailableVaults();
      const totalTVL = vaults.reduce((sum, vault) => sum + (Number((vault as any).tvl) || 0), 0);
      ...
      return this.createErrorResult("troves_tvl", `Failed to get TVL: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}
