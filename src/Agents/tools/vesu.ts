...
export class VesuTool extends BaseTool<VesuPayload> {
  metadata: ToolMetadata = {
    name: "vesu_tool",
    description: "DeFi lending and borrowing operations via Vesu - lend, borrow, manage positions, and check health",
    parameters: {
      operation: {
        type: "string",
        description: "The operation to perform (e.g., 'lend', 'borrow', 'withdraw', 'repay', 'get_pools', 'get_positions', 'get_quote', 'check_health')",
        required: true,
        enum: [
          "lend", "borrow", "withdraw", "repay", "get_pools", "get_positions",
          "get_quote", "check_health", "get_pool_stats", "get_pool_by_asset",
          "check_health_factor"
        ],
      },
      asset: {
  ...
  async execute(payload: VesuPayload, userId: string): Promise<ToolResult> {
    const { operation, asset, amount, poolId, positionId, collateralAsset, borrowAsset, collateralAmount, borrowAmount, userAddress, timeHorizon } = payload;
    ...
    try {
      ...
      switch (operation) {
        ...
        case "get_positions":
          if (!userAddress) return this.createErrorResult("vesu_positions", "User address is required.");
          const positions = await this.vesuService.getUserPositions(userAddress);
          return this.createSuccessResult("vesu_positions", { userAddress, positions });

        case "get_quote":
          if (!amount) return this.createErrorResult("vesu_quote", "Amount is required for quote.");
          const quote = await this.vesuService.getBestYieldQuote(amount.toString(), timeHorizon);
          return this.createSuccessResult("vesu_quote", { amount, timeHorizon, quote });

        case "lend":
          if (!account || !poolId || !amount) return this.createErrorResult("vesu_lend", "Account, pool ID, and amount are required for lending.");
          const lendOperation = { 
            poolId, 
          ...
          const lendResult = await this.vesuService.executeLendingOperation(lendOperation, account);
          return this.createSuccessResult("vesu_lend", { lendResult });

        case "borrow":
          if (!account || !poolId || !amount) return this.createErrorResult("vesu_borrow", "Account, pool ID, and amount are required for borrowing.");
          ...
          const borrowResult = await this.vesuService.executeLendingOperation(borrowOperation, account);
          return this.createSuccessResult("vesu_borrow", { borrowResult });

        case "withdraw":
          if (!account || !poolId || !amount) return this.createErrorResult("vesu_withdraw", "Account, pool ID, and amount are required for withdrawal.");
          ...
          const withdrawResult = await this.vesuService.executeLendingOperation(withdrawOperation, account);
          return this.createSuccessResult("vesu_withdraw", { withdrawResult });

        case "repay":
          if (!account || !poolId || !amount) return this.createErrorResult("vesu_repay", "Account, pool ID, and amount are required for repayment.");
          ...
          const repayResult = await this.vesuService.executeLendingOperation(repayOperation, account);
        ...
        case "get_pool_by_asset":
          if (!asset) return this.createErrorResult("vesu_pool_by_asset", "Asset is required.");
          const poolByAsset = await this.vesuService.getPoolByAsset(asset);
          return this.createSuccessResult("vesu_pool_by_asset", { asset, pool: poolByAsset });

        case "check_health_factor":
          if (!userAddress) return this.createErrorResult("vesu_check_health_factor", "User address is required.");
          const healthFactor = await this.vesuService.checkHealthFactor(userAddress);
          return this.createSuccessResult("vesu_check_health_factor", { userAddress, healthFactor });

        default:
          return this.createErrorResult("vesu_tool", `Unknown operation: ${operation}`);
      }
    } catch (error) {
      memoryStore.add(userId, `Error in VesuTool: ${error instanceof Error ? error.message : 'Unknown error'}`);
