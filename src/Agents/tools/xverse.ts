...
export class XverseTool extends BaseTool<XversePayload> {
  metadata: ToolMetadata = {
    name: "xverse_tool",
    description: "Bitcoin wallet operations via Xverse - balance, transactions, Ordinals, Runes, and BRC-20 tokens",
    parameters: {
      operation: {
        type: "string",
  ...
  private async getBalance(payload: XversePayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult("xverse_balance", "Address is required for balance check");
    }

    try {
      const balance = await this.xverseService.getAddressBalance(payload.address);
    return this.createSuccessResult("xverse_balance", {
      ...
      return this.createErrorResult("xverse_balance", `Failed to get balance: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getTransactions(payload: XversePayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult("xverse_transactions", "Address is required for transaction history");
    }

    try {
      const transactions = await this.xverseService.getAddressTransactions(payload.address);
      ...
      return this.createErrorResult("xverse_transactions", `Failed to get transactions: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getUTXOs(payload: XversePayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult("xverse_utxos", "Address is required for UTXO check");
    }
    ...
    try {
      ...
      return this.createErrorResult("xverse_utxos", `Failed to get UTXOs: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getPrice(): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("xverse_price", `Failed to get Bitcoin price: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async sendBitcoin(payload: XversePayload): Promise<ToolResult> {
    if (!payload.toAddress || !payload.amount) {
      return this.createErrorResult("xverse_send", "Recipient address and amount are required");
    }
    ...
    try {
      ...
      return this.createErrorResult("xverse_send", `Failed to prepare Bitcoin transaction: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async createWallet(): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("xverse_wallet", `Failed to create wallet: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async validateAddress(payload: XversePayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult("xverse_validate", "Address is required for validation");
    }
    ...
    try {
      ...
      return this.createErrorResult("xverse_validate", `Failed to validate address: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getOrdinals(payload: XversePayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult("xverse_ordinals", "Address is required for Ordinals check");
    }

    try {
      const ordinals = await this.xverseService.getOrdinalsByAddress(payload.address);
      ...
      return this.createErrorResult("xverse_ordinals", `Failed to get Ordinals: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getOrdinalById(payload: XversePayload): Promise<ToolResult> {
    if (!payload.inscriptionId) {
      return this.createErrorResult("xverse_ordinal", "Inscription ID is required");
    }

    try {
      const ordinal = await this.xverseService.getOrdinalById(payload.inscriptionId);
      return this.createSuccessResult("xverse_ordinal", {
        ordinal: ordinal,
        message: `Ordinal details for ${payload.inscriptionId}`
      });
    } catch (error) {
      return this.createErrorResult("xverse_ordinal", `Failed to get Ordinal: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getOrdinalCollections(payload: XversePayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult("xverse_collections", "Address is required for collections check");
    }

    try {
      const collections = await this.xverseService.getOrdinalCollectionsByAddress(payload.address);
      ...
      return this.createErrorResult("xverse_collections", `Failed to get collections: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getTopOrdinals(): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("xverse_top_ordinals", `Failed to get top Ordinals: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getRunes(payload: XversePayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult("xverse_runes", "Address is required for Runes check");
    }

    try {
      const runes = await this.xverseService.getRunesByAddressV2(payload.address);
      ...
      return this.createErrorResult("xverse_runes", `Failed to get Runes: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getRuneById(payload: XversePayload): Promise<ToolResult> {
    ...
    try {
      ...
      return this.createErrorResult("xverse_rune", `Failed to get Rune: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async searchRunes(payload: XversePayload): Promise<ToolResult> {
    if (!payload.query) {
      return this.createErrorResult("xverse_search", "Search query is required");
    }
    ...
    try {
      ...
      return this.createErrorResult("xverse_search", `Failed to search Runes: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getTopRunes(): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("xverse_top_runes", `Failed to get top Runes: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getRuneGainersLosers(): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("xverse_gainers_losers", `Failed to get gainers/losers: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getBRC20Balances(payload: XversePayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult("xverse_brc20", "Address is required for BRC-20 balances");
    }

    try {
      const balances = await this.xverseService.getBRC20BalancesByAddress(payload.address);
      ...
      return this.createErrorResult("xverse_brc20", `Failed to get BRC-20 balances: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getBRC20History(payload: XversePayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult("xverse_brc20_history", "Address is required for BRC-20 history");
    }

    try {
      const history = await this.xverseService.getBRC20TransactionHistoryByAddress(payload.address);
      ...
      return this.createErrorResult("xverse_brc20_history", `Failed to get BRC-20 history: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getBRC20ByTicker(payload: XversePayload): Promise<ToolResult> {
    if (!payload.ticker) {
      return this.createErrorResult("xverse_brc20_ticker", "Ticker is required for BRC-20 token");
    }
    ...
    try {
      ...
      return this.createErrorResult("xverse_brc20_ticker", `Failed to get BRC-20 token: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getSparkBalances(payload: XversePayload): Promise<ToolResult> {
    if (!payload.address) {
      return this.createErrorResult("xverse_spark", "Address is required for Spark balances");
    }

    try {
      const balances = await this.xverseService.getSparkBalancesByAddress(payload.address);
      ...
      return this.createErrorResult("xverse_spark", `Failed to get Spark balances: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getMempoolFees(): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("xverse_fees", `Failed to get network fees: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getCurrentBlock(): Promise<ToolResult> {
    try {
      ...
      return this.createErrorResult("xverse_current_block", `Failed to get current block: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async getBlockByHeight(payload: XversePayload): Promise<ToolResult> {
    ...
    try {
      ...
      return this.createErrorResult("xverse_block", `Failed to get block: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}
