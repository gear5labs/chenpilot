...
router.get('/transactions/:address', async (req, res) => {
  try {
    ...
    const transactions = await xverseService.getAddressTransactions(
      address,
      Number(limit)
    );
...
router.get('/utxos/:address', async (req, res) => {
  try {
    ...
    const utxos = await xverseService.getAddressUTXOs(
      address,
      Number(offset),
      Number(limit)
    );
...
router.get('/ordinals/:address', async (req, res) => {
  try {
    ...
    const ordinals = await xverseService.getOrdinalsByAddress(
      address,
      Number(offset),
      Number(limit)
    );
...
router.get('/collections/:address', async (req, res) => {
  try {
    ...
    const collections = await xverseService.getOrdinalCollectionsByAddress(
      address,
      Number(offset),
      Number(limit)
    );
...
router.get('/ordinals/top', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : 'Failed to get top Ordinals',
    });
  }
});
...
router.get('/runes/:address', async (req, res) => {
  try {
    ...
    const runes = await xverseService.getRunesByAddressV2(
      address,
      Number(offset),
      Number(limit)
    );
...
router.get('/runes/gainers-losers', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : 'Failed to get gainers/losers',
    });
  }
});
...
router.get('/brc20/:address', async (req, res) => {
  try {
    ...
    const balances = await xverseService.getBRC20BalancesByAddress(
      address,
      Number(offset),
      Number(limit)
    );
...
router.get('/brc20/history/:address', async (req, res) => {
  try {
    ...
    const history = await xverseService.getBRC20TransactionHistoryByAddress(
      address,
      Number(offset),
      Number(limit)
    );
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : 'Failed to get BRC-20 history',
    });
  }
});
...
router.get('/brc20/token/:ticker', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : 'Failed to get BRC-20 token',
    });
  }
});
...
router.get('/spark/:address', async (req, res) => {
  try {
    ...
    const balances = await xverseService.getSparkBalancesByAddress(
      address,
      Number(offset),
      Number(limit)
    );
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : 'Failed to get Spark balances',
    });
  }
});
...
router.get('/fees', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : 'Failed to get mempool fees',
    });
  }
});
...
router.get('/block/current', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : 'Failed to get current block',
    });
  }
});
...
router.post('/transaction/mnemonic', async (req, res) => {
  try {
    ...
    const signedTransaction = await xverseService.createTransactionWithMnemonic(
      transactionRequest,
      mnemonic
    );
...
router.post('/address/mnemonic', async (req, res) => {
  try {
    ...
    const address = xverseService.getAddressFromMnemonic(
      mnemonic,
      derivationPath
    );
    ...
    res.status(500).json({
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Failed to get address from mnemonic',
    });
  }
});
  ...
  try {
    ...
    res.status(500).json({
      ...
      error:
        error instanceof Error
          ? error.message
          : 'Failed to process Bitcoin command',
    });
  }
});
