...
router.post("/swap", async (req, res) => {
  try {
    const { userId, fromAsset, toAsset, amount, fromChain, toChain, recipientAddress } = req.body;
    
    if (!userId || !fromAsset || !toAsset || !amount) {
      throw new ValidationError("UserId, from asset, to asset, and amount are required");
    }
    ...
    if (fromAsset === "BTC" && toAsset === "STRK") {
      ...
      throw new ValidationError(`Unsupported asset pair: ${fromAsset} -> ${toAsset}`);
    }
...
router.get("/refundable/:userId", async (req, res) => {
  try {
    ...
    const refundableSwaps = await atomiqService.getRefundableSwaps(chain as string || "STARKNET", userId);
    res.json({ 
      success: true, 
      data: {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : "Failed to get refundable swaps"
    });
  }
});
...
router.post("/spendable-balance", async (req, res) => {
  try {
    ...
    const spendableBalance = await atomiqService.getSpendableBalance(signer, token);
    res.json({ 
      success: true, 
      data: {
        spendableBalance,
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : "Failed to get spendable balance"
    });
  }
});
...
router.get("/bitcoin-balance/:address", async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : "Failed to get Bitcoin spendable balance"
    });
  }
});
...
router.get("/limits", async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : "Failed to get swap limits"
    });
  }
});
...
router.post("/cross-chain", async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : "Failed to process cross-chain command"
    });
  }
});
