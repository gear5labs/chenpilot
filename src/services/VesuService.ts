...
export class VesuService {
  ...
  constructor() {
    ...
    this.config = {
      ...
      contractAddresses: {
        ...
        re7UsdcFrontier: "0x5c03e7e0ccfe79c634782388eb1e6ed4e8e2a013ab0fcc055140805e46261bd",
        re7xBtc: "0x3a8416bf20d036df5b1cf3447630a2e1cb04685f6b0c3a70ed7fb1473548ecf",
        re7UsdcStableCore: "0x73702fce24aba36da1eac539bd4bae62d4d6a76747b7cdd3e016da754d7a135"
      },
      supportedAssets: ["ETH", "STRK", "USDC", "USDT", "WBTC", "wstETH", "EKUBO", "xSTRK"]
    };
  }
  ...
  async getAvailablePools(): Promise<VesuPool[]> {
    try {
      ...
      const transformedPools: VesuPool[] = pools.map((pool: any) => {
        ...
        const supplyApyDecimal = Number(asset.stats.supplyApy.value) / Math.pow(10, asset.stats.supplyApy.decimals);
        const apy = supplyApyDecimal * 100; // Convert to percentage

        // Convert utilization rate
        const utilizationDecimal = Number(asset.stats.currentUtilization.value) / Math.pow(10, asset.stats.currentUtilization.decimals);
        ...
        return {
          ...
          isActive: asset.stats.canBeBorrowed && (!pool.shutdownMode || pool.shutdownMode === "none"),
      ...
      for (const pool of pools) {
        ...
        try {
          ...
          if (vTokenBalance && Number(vTokenBalance.balance) > 0) {
            ...
            const collateralValue = (Number(suppliedAmount) / Math.pow(10, pool.decimals)) * (pool.usdPrice || 0);
  ...
  async getBestYieldQuote(amount: string, timeHorizon: number = 30): Promise<VesuQuote> {
    try {
      const pools = await this.getAvailablePools();
      ...
      const estimatedYield = amountBN * BigInt(Math.floor(apyDecimal * timeFraction * 10000)) / BigInt(10000);
  ...
  async executeLendingOperation(operation: VesuLendingOperation, account: Account): Promise<{ success: boolean; transactionHash?: string; error?: string }> {
    try {
      if (!this.provider) {
        throw new Error("VesuService not initialized");
      }
      ...
      if (!targetPool || !targetPool.vTokenAddress) {
        throw new Error(`Pool not found or vToken address not available for pool: ${poolId}`);
      }
  ...
  async checkHealthFactor(userAddress: string): Promise<VesuHealthCheck> {
    try {
      ...
      const healthFactor = totalDebt > 0 ? Number(totalCollateral) / Number(totalDebt) : 999.99;
  ...
  async getPoolStats(): Promise<{
    ...
    try {
      ...
      const totalLiquidity = pools.reduce((sum, pool) => {
        const liquidity = Number(pool.totalLiquidity) / Math.pow(10, pool.decimals);
        return sum + liquidity;
      }, 0);

      const averageUtilization = pools.reduce((sum, pool) => sum + pool.utilizationRate, 0) / pools.length;
