...
export class StarknetService {
  ...
  private formatAddress(address: string): string {
    ...
    if (formattedAddress.length !== 66) {
      throw new Error(
        `Invalid Starknet address length: ${formattedAddress.length}, expected 66 characters (including 0x prefix)`
      );
    }
    ...
    if (!/^0x[0-9a-fA-F]{64}$/.test(formattedAddress)) {
      throw new Error(
        'Invalid Starknet address format: must be 64 hex characters after 0x prefix'
      );
    }
  ...
  async createAccount(): Promise<StarknetAccountData> {
    try {
      ...
      throw new Error(
        `Failed to create Starknet account: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
   ...
   */
  async deployAccount(
    accountData: StarknetAccountData
    ...
    try {
      ...
      if (BigInt(balance) < BigInt(deploymentCost)) {
        throw new Error(
          `Insufficient balance for deployment. Required: ${deploymentCost}, Available: ${balance}`
        );
      }

      console.log(
        `Deploying account ${accountData.precalculatedAddress} with balance: ${balance}`
      );
  ...
  async getAccountBalance(address: string): Promise<string> {
    try {
      ...
      const erc20Address =
        process.env.STARKNET_ERC20_ADDRESS ||
   ...
   */
  async isAccountFunded(
    address: string,
    minimumBalance: string = '1000000000000000000'
  ): Promise<boolean> {
    try {
  ...
  async getDeploymentCost(): Promise<string> {
    try {
      ...
      throw new Error(
        `Failed to get deployment cost: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getRecentTransactions(
    ...
    try {
      ...
      throw new Error(
        `Failed to get recent transactions: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getNativeBalance(address: string): Promise<string> {
    try {
      const formattedAddress = this.formatAddress(address);
      console.log(
        `Native balance check for ${formattedAddress}: 0 (not implemented)`
      );
      return '0';
    } catch (error) {
   ...
   */
  async hasReceivedFunding(
    address: string
  ): Promise<{ funded: boolean; balance: string; transactionHash?: string }> {
    try {
      const balance = await this.getAccountBalance(address);
