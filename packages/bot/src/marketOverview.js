...
class MarketOverviewService {
    ...
    fetchMarketOverview() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                // Fetch top gainers
                const gainersRes = yield fetch(`${this.BACKEND_URL}/api/assets/trending?sort=price_change&order=desc&limit=5`);
                const topGainers = gainersRes.ok ? yield gainersRes.json() : [];
                // Fetch top losers
                const losersRes = yield fetch(`${this.BACKEND_URL}/api/assets/trending?sort=price_change&order=asc&limit=5`);
                const topLosers = losersRes.ok ? yield losersRes.json() : [];
                // Fetch top by volume
                const volumeRes = yield fetch(`${this.BACKEND_URL}/api/assets/trending?sort=volume&order=desc&limit=5`);
                const topVolume = volumeRes.ok ? yield volumeRes.json() : [];
                // Fetch network status
                let networkStatus;
                try {
                    const statusRes = yield fetch(`${this.BACKEND_URL}/api/network/status`);
                    if (statusRes.ok) {
                        const statusData = yield statusRes.json();
                        networkStatus = {
                            isHealthy: ((_a = statusData.health) === null || _a === void 0 ? void 0 : _a.isHealthy) || false,
                            latestLedger: ((_b = statusData.health) === null || _b === void 0 ? void 0 : _b.latestLedger) || 0
                        };
                    }
                }
                catch (_c) {
    ...
    formatMarketOverviewMessage(data) {
        ...
        if (data.topGainers.length === 0) {
            ...
            for (const asset of data.topGainers) {
                const change = asset.priceChange24h >= 0 ? `+${asset.priceChange24h.toFixed(2)}%` : `${asset.priceChange24h.toFixed(2)}%`;
