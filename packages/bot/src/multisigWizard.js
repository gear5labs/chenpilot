...
class MultisigWizard {
    ...
    getStepMessage(state, additionalInfo) {
        ...
        switch (state.step) {
            ...
            case 3:
                const currentCount = ((_a = state.config.signers) === null || _a === void 0 ? void 0 : _a.length) || 0;
                return `🔐 **Multisig Setup Wizard**\n\n` +
    ...
    getSummaryMessage(config) {
        const signersList = config.signers.map((s, i) => `${i + 1}. \`${s.key.slice(0, 8)}...\` (weight: ${s.weight})`).join('\n');
        const totalWeight = config.signers.reduce((sum, s) => sum + s.weight, 0);
